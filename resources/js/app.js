...
var newMsgaudio = new Audio("/audio/new_msg.mp3");
...
Vue.component("easin-chat", require("./components/chat/Container.vue").default);
// New page Builder
...
Vue.component('jobfilter-section', require('./components/pages/job/JobFilter.vue').default);
...
{
    if (Vue.prototype.$isLoggedIn == "true"){
        const myHeader = new Vue({
            el: "#wt-header",
            store: _Store,
            components:{
                VueMomentsAgo,
            ...
            mounted() {
                this.$store.dispatch("Conversation/getAllConversation");
                // Flash message
                ...
                // Run pusher to check notification
                window.Echo.private(`User-channel.${window.USERID}`).listen(`.user-notification`, (data)=>{
                    this.userNotifications.unshift(data.notification);
                    if(data.notification.seen == "0"){
                        this.unseenNotification+=1;
                    }
                });

                window.Echo.private(`Message-notification.${window.USERID}`).listen(`.user-message`,(data)=>{
                    if (data.type == "new_conversation_created"){
                        var msgData = {
                            type : data.type,
                            msg: data.data
                        }
                        this.$store.commit("Conversation/PUSH_NEW_CONVERSATION",msgData)
                    }
                    else if(data.type == "new_msg"){
                        this.$store.commit("Conversation/RECIEVE_NEW_MSG",data);
                    }
            ...
            methods:{
                getAllNotification(){
                    axios.get(APP_URL +"/user-notifications").then(resp=>{
                        return resp.data;
                    }).then(data=>{
                        console.log(data);
                        this.userNotifications = data.data;
                        this.unseenNotification = data.unseen;
                    }).catch(err=>{
                        console.error(err.response.data);
                    })
                },
                notificationOpen(id,index){
                    if(this.userNotifications[index].seen == "0"){
                        axios.post("/user-has-seen-notification",{
                            id: id,
                        }).then(resp=>{
                            return resp.data;
                        }).then(data=>{
                        ...
                        this.userNotifications[index].seen = "1";
                        if(this.unseenNotification > 0){
                            this.unseenNotification -= 1;
                        }
                    }
                },
...
if (document.getElementById("modalSignIn")){
        ...
        methods :{
            signIn(){
                ...
                axios.post(APP_URL + "/user-login",{
                    ...
                    if(data.status == "password_missmatch"){
                        this.passError.error = true;
                        this.passError.msg = data.msg;
                    }
                    else if(data.status == "unverified_user"){
                        this.needVerify = true;
                        this.verifyMsg = data.msg;
                    }
                    ...
                    if(errdata.email[0]){
                        this.errEmail.error = true;
                        this.errEmail.msg = errdata.email[0];
                    }
                    if(errdata.password[0]){
                        this.passError.error = true;
                        this.passError.msg = errdata.password[0];
                    }
                })
            },
            resetLoginError(){
                this.errEmail.error = false;
                this.errEmail.msg = "";

                this.passError.error = false;
                this.passError.msg = "";
            ...
            resendVerifyCode(){
                this.sendingCodeLoader = true;
                axios.post(APP_URL + "/send-verification-code",{
                    email: this.loginForm.email
                }).then(resp=>{
                    return resp.data;
                }).then(data=>{
                    this.sendingCodeLoader = false;
                    if(data.status == "ok"){
                        this.codeSent = true;
                    }
                }).catch(err=>{
            ...
            verifyCode(){
                ...
                axios.post("/verify-code",{
                    ...
                    if (data.status == "invalid_code"){
                        ...
                        this.codeError.msg = data.msg;
                    }
                    else if(data.status == "ok"){
                        this.verificationSuccess = true;
                        this.verificationSuccessMsg = data.msg;
                        this.needVerify = false;
                    }
                }).catch(err=>{
                    console.error(err.response.data);
                    this.$swal("Failed","Verification failed","error");
                    let errdata = err.response.data.errors;
                    if(errdata.code[0]){
                        this.codeError.error=true;
                        this.codeError.msg = errdata.code[0];
                    }
                })
            }
        },
                ...
                axios.post(APP_URL + '/freelancer/store-profile-settings', form_data,{
                    ...
                    .catch(function (error) {
                        ...
                        if(error.response.data.errors.dateOfBirth){
                            self.showError(error.response.data.errors.dateOfBirth[0]);
                        }
                ...
                axios.post(APP_URL + '/job/post-job', form_data)
                    ...
                    .catch(function (error) {
                        ...
                        if (error.response.data.errors.job_duration) {
                            ...
                            self.showError("Bitte Jobdauer angeben");
                        ...
                        if (error.response.data.errors.title) {
                            ...
                            self.showError("Berufsbezeichnung ist erfoderlich");
                        ...
                        if (error.response.data.errors.project_cost) {
                            ...
                            self.showError("Budget festlegen");

                        }

                        if (error.response.data.errors.description) {

                            self.showError("Bitte füge eine Job beschreibung");
                        ...
                        if (error.response.data.errors.locations) {
                            ...
                            self.showError(error.response.data.errors.locations);
    ...
    const jobVue = new Vue({
        ...
        data: {
            ...
            proposal: {
                ...
                /*amountString: Vue.prototype.trans('lang.enter_proposal_amount'),*/
        ...
        methods: {
            ...
            calculate_amount: function (commission) {
                ...
                let amount = this.filter_price(this.proposal.amountString);

                this.proposal.amount = amount;

                this.proposal.deduction = (amount / 100) * commission;

                this.proposal.total = amount - this.proposal.deduction;

            },

            filter_price : function(text) {
            ...
            submitJobProposal: function (id, user_id) {
                ...
                form_data.append("amount",this.proposal.amount);
...
if (document.getElementById("manageJob"))
{
    const manageJob = new Vue({
        el: "#manageJob",
        data() {
            return {
        ...
        methods: {
            deleteProject(projectId) {
                this.$swal({
                    ...
                    {
                        axios.post("/employer/delete-job",{
                            jobId: projectId
                        }).then(resp=>{
                            return resp.data;
                        }).then(data=>{
                            console.log(data);
                            if(data.status == "ok") {
                                this.$swal("Success",data.msg,"success").then(()=>{
                                    window.location.reload();
                                });
                            }
                            else {
                                this.$swal("Failed",data.msg,"error");
                            }
                        }).catch(err=>{
                            console.error(err.response.data);
                        });
            ...
            completeJob(jobId) {
                axios.post("/employer/complete-job",{
                    jobId: jobId
                }).then(resp=>{
                    return resp.data;
                }).then(data=>{
