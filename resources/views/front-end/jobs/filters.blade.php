                ...
                <div class="form-group">
                    <select class="chosen-select" name="category">
                        ...
                        @foreach ($categories as $category)
                            ...
                            @php $checked = ( !empty($_GET['category']) && in_array($category->slug, $_GET['category'] )) ? 'selected' : ''; @endphp
                            <option {{$checked}}>{{ $category->title }}</option>

                        @endforeach
                    </select>
                    <i style="z-index: 2; margin-top: -12px" class="icon fas fa-briefcase"></i>
                </div>
            </div>
