...

{
...

    {
        ...
        if (!empty($image)) {
            ...
            if ($extension === "jpg" || $extension === "png" || $extension === "svg") {
                ...
                {
                    // Original
                    $image->move($temp_path,$file_original_name);
                }
...

    {
        ...
        array_push($list,array("title"=>"Kategorie","value"=>"category"));



        $ctgs = DB::table('categories')->get();

        foreach($ctgs as $ctg)

        {

            array_push($list,array("title"=>$ctg->title,"value"=>$ctg->title));
    ...
    public static function getSearchableListV2($json)

    {

        $json = array();

        $json = DB::table('categories')->select('title as name')->get()->toArray();
...

    {
        ...
        if (!empty($size)) {
            ...
            return !empty($profile_image) ? '/uploads/users/' . $user_id . '/' . $profile_image : null;
    ...
    public static function getUserMemberSince($id)
    {
...

    {
        ...
        echo '<option value="">'. trans('Kanton auswählen'). '</option>';
