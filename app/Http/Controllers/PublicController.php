...

{
    ...
    public function getSearchResult(Request $req, $search_type = "")

    {
        ...
        $search_datePosted = $req->datePosted??"";
        ...
        if (!empty($_GET['type'])) {
            ...
            if ($type == 'employer' || $type == 'freelancer') {
                ...
                $saved_jobs = !empty(auth()->user()->profile->saved_jobs) ? unserialize(auth()->user()->profile->saved_jobs) : array();
    ...
    public function resultTwo(Request $req)
    {
        $json = array();
        ...
        $search_categories = $req->category;

        $search_locations = $req->locations;

        $search_project_lengths = $req->project_lengths;
        ...
        try {
            ...
            if(!empty($search_categories)){
                $filteredJobIds = DB::table("catables")->whereIn("category_id", $search_categories)->pluck("catable_id")->toArray();
                $jobs->where("id", $filteredJobIds);
            }
        }
        ...
        $jobs = $jobs->orderByRaw("is_featured DESC, updated_at DESC")->with(["employer", "location"])->paginate(10);
