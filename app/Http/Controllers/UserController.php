...

{
        ...
        if (!empty($request)) {
            ...
            if (!empty($type) && !empty($product_id)) {
                ...
                DB::table('orders')
                    ...
                    ->update(['invoice_id' => $invoice_id, "status" => "completed"]);
                ...
                if ($orderData->type == 'job') {

                    $proposal = Proposal::find($orderData->product_id);
                    ...
                    $userNotification["send_notification_to"] = $proposal->freelancer_id;

                    $empName = Helper::getUserName($job->user_id);

                    $notification = new UserNotifications();
                    $notification->user_id = $proposal->freelancer_id;
                    $notification->type = "job_hired";
                    $notification->body = "'$empName' "."hired you for the job - ".$title;
                    $notification->url = route("jobDetail",$job->slug);
                    $notification->save();

                    $userNotification["data"] = $notification;



                    // Create new message
                    $conversation = new Conversation();
                    $conversation->employer_id = $job->user_id;
                    $conversation->freelancer_id = $proposal->freelancer_id;
                    $conversation->created_by = Auth::user()->id;
                    $conversation->type = $orderData->type;
                    $conversation->project_id = $job->id;
                    $conversation->save();

                    // build message notification model
                    $msgNotify = array(
                        "id" => $conversation->id,
                        "participant" => array(
                    ...
                    $userMsgNotification["send_to_user"] = $proposal->freelancer_id;
                    $userMsgNotification["type"] = "new_conversation_created";
                    ...
                    broadcast(new \App\Events\UserNotification($userNotification));
                ...
                if ($orderData->type == 'package') {
                    ...
                    if (Schema::hasColumn('items', 'type')) {
                        ...
                        $item = DB::table('items')->select('id')->where('type', 'package')->where('subscriber', $orderData->user_id)->first();

                        if (empty($item)) {

                            $item = DB::table('items')->select('id')->where('subscriber', $orderData->user_id)->first();
                        }
                    } else {
                    ...
                    $item = DB::table('items')->select('id')->where('invoice_id', $orderData->invoice->id)->first();
                ...
                $item->invoice_id = filter_var($orderData->invoice->id, FILTER_SANITIZE_NUMBER_INT);

                $item->product_id = filter_var($orderData->product_id, FILTER_SANITIZE_NUMBER_INT);

                $item->subscriber = $orderData->user_id;
                ...
                // Send freelancer a notification
    ...
    public function userNotification(Request $req)
    {
        ...
        $data = UserNotifications::where("user_id",$userId)->orderBy("id","desc")
        ->with("sender:id,user_id,avater")
        ...
        foreach($data as $d)
        {
            $d->user_init = ucfirst(substr($d->user->first_name,0,1)). ucfirst(substr($d->user->last_name,0,1));
            $d->user_init_color = $nameColor->getColor(ucfirst(substr($d->user->first_name,0,1)));
            if($d->seen == 0)
            {
                $totalUnseen += 1;
            }
    ...
    public function makeNotificationSeen(Request $req)
    {
        $this->validate($req,[
            "id" => "required|numeric|exists:user_notifications,id",
        ]);

        $data = UserNotifications::find($req->id);
        $data->seen = 1;
        $data->save();
        return [
